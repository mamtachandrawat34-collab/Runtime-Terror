...
function drawCracks() {
  ...
  for (let c of cracks) {
    ...
    line(c.x2, c.y2, c.x2 + cos(c.a + 0.7) * c.len * 0.45, c.y2 + sin(c.a + 0.7) * c.len * 0.45);
  }
...
function drawFestivalLights() {
  ...
  for (let i = 0; i < 12; i++) {
    let x = i * 75 + 10;
    let c = color(120 + sin(frameCount * 0.06 + i) * 120, 100 + sin(frameCount * 0.08 + i * 2) * 110, 180 + sin(frameCount * 0.05 + i) * 70);
